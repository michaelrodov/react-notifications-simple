...
export default class Notification extends React.Component {
    constructor(props) {
        super(props);
        let types = [{type: "error", icon: "fa-exclamation-triangle"},
            {type: "warning", icon: "fa-exclamation"},
            {type: "info", icon: "fa-info"}];
    ...
    render() {
        ...
        return (
            <span id={"container"} className={"flex flex__row flex__justify-between"}>
                <span className={"flex flex__row flex__justify_start"}>
                    {iconDom}
                    <span id={"message"} className={"notification-message"}>{this.props.message}</span>
                </span>
